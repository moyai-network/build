...
import (
	...
	md "github.com/df-mc/dragonfly/server/block/model"
...
func (b Bed) UseOnBlock(pos cube.Pos, face cube.Face, _ mgl64.Vec3, w *world.World, user item.User, ctx *item.UseContext) (used bool) {
	...
	if _, ok := w.Block(pos.Side(cube.FaceDown)).Model().(md.Solid); !ok {
		return
	}
	...
	if _, ok := w.Block(sidePos.Side(cube.FaceDown)).Model().(md.Solid); !ok {
		return
	}
